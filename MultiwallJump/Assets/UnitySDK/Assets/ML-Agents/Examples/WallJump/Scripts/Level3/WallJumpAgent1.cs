...
public class WallJumpAgent1 : Agent
{
    ...
    Bounds spawnAreaBounds2;

    //  DeadPit
    public GameObject deadPit;
    public GameObject deadPit2;

    public GameObject deadPitSpawnArea;
    ...
    public GameObject shortBlock2;
    public GameObject wall;
    Rigidbody shortBlockRB;
    Rigidbody shortBlockRB2;
    Rigidbody agentRB;
    ...
    public List<GameObject> coinGO = new List<GameObject>();
    ...
    public override void InitializeAgent()
    {
        ...
        shortBlockRB2 = shortBlock2.GetComponent<Rigidbody>();
        // deadPitRB = deadPit.GetComponent<Rigidbody>();
        spawnAreaBounds = spawnArea.GetComponent<Collider>().bounds;
        spawnAreaBounds2 = spawnArea2.GetComponent<Collider>().bounds;
    ...
    public Vector3 GetRandomSpawnPos2()
    {
        Vector3 randomSpawnPos = Vector3.zero;
        float randomPosX = Random.Range(-spawnAreaBounds2.extents.x,
                                        spawnAreaBounds2.extents.x);
        float randomPosZ = Random.Range(-spawnAreaBounds2.extents.z,
                                        spawnAreaBounds2.extents.z);

        randomSpawnPos = spawnArea2.transform.position +
                                  new Vector3(randomPosX, 0.45f, randomPosZ);
        return randomSpawnPos;
    }
    ...
    public Vector3 SpawnRandomCoins()
    {
        ...
        randomCoinPos = ground.transform.position + new Vector3(Random.Range(-groundArea.extents.x, groundArea.extents.x),
                                    2.0f,
                                    Random.Range(-groundArea.extents.z, groundArea.extents.z));
    ...
    public override void AgentAction(float[] vectorAction, string textAction)
    {
        ...
        if ((!Physics.Raycast(agentRB.position, Vector3.down, 20))
            ...
            || (!Physics.Raycast(shortBlockRB2.position, Vector3.down, 20))))
        {
            Done();
            SetReward(-1f);
            ResetBlock();
    ...
    private void OnTriggerEnter(Collider col)
    {
        if (col.gameObject.CompareTag("coin"))
        {
            ...
            AddReward(0.5f);
            // coins.transform.position = new Vector3(-23.0f, 2.0f, 10.0f);
        }
    ...
    void ResetBlock()
    {
        shortBlockRB.transform.position = GetRandomSpawnPos();
        shortBlockRB.velocity = Vector3.zero;
        shortBlockRB.angularVelocity = Vector3.zero;

        shortBlockRB2.transform.position = GetRandomSpawnPos2();
        shortBlockRB2.velocity = Vector3.zero;
        shortBlockRB2.angularVelocity = Vector3.zero;

        deadPit.transform.position = GetRandomPitPos();
        deadPit2.transform.position = GetRandomPitPos();

        // deadPitRB.velocity = Vector3.zero;
        // deadPitRB.angularVelocity = Vector3.zero;
    ...
    void RemoveCoins()
    {
        // if (coinGO != null)
        // {
        //     for (int i = 0; i < coinGO.Count; i++)
        //     {
        //         GameObject.Destroy(coinGO[i]);
        //         // coinGO.RemoveAt(i);
        //     }
        // }
        // else
        ...
        for (int i = 0; i < coinGO.Count; i++)
        {
            GameObject coin = (GameObject)Instantiate(coinGO[i], SpawnRandomCoins(), Quaternion.identity);
            Destroy(coin, 4);
        }
        // }
    ...
    public override void AgentReset()
    {
        ...
        if (dog != null)
        {
            dog.transform.position = SpawnRandomCoins() - new Vector3(0,1,0);
        }
        transform.localPosition = new Vector3(
            36 * (Random.value - 0.5f), 1, -21 + (Random.Range(0, 2)));
        configuration = Random.Range(0, 5);
