...
public class WallJumpAgent : Agent
{
    ...
    {
        Vector3 randomSpawnPos2 = Vector3.zero;
        float randomPosX2 = Random.Range(-spawnAreaBounds2.extents.x,
                                        spawnAreaBounds2.extents.x);
        float randomPosZ2 = Random.Range(-spawnAreaBounds2.extents.z,
                                        spawnAreaBounds2.extents.z);

        randomSpawnPos2 = spawnArea2.transform.position +
                                    new Vector3(randomPosX2, 0.45f, randomPosZ2);
        return randomSpawnPos2;
    }
    ...
    public override void AgentAction(float[] vectorAction, string textAction)
    {
            ...
            || (!Physics.Raycast(shortBlockRB2.position, Vector3.down, 20)))
        {
            Done();
            SetReward(-1f);
            ResetBlock(0,shortBlockRB);
            ResetBlock(1,shortBlockRB2);
            StartCoroutine(
    ...
    void ResetBlock(int number, Rigidbody blockRB)
    {
        if (number == 0)
        {
            ...
            blockRB.transform.position = GetRandomSpawnPos2();
            blockRB.velocity = Vector3.zero;
            blockRB.angularVelocity = Vector3.zero;
        }
    ...
    void ConfigureAgent(int config)
    {
        if (config == 0)
        {
            ...
            wall2.transform.localScale = new Vector3(
                wall2.transform.localScale.x,
                academy.resetParameters["no_wall_height"],
                wall2.transform.localScale.z);
